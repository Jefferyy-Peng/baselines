...
def eval_concept_localization(net, val_path, ckpt_path, log_dir, device, activation):
    ...
    concept_texts = ['peripheral zone', 'transition zone']
    ...
    with torch.no_grad():
        for step, (sample, subject_id, slice_id) in enumerate(tqdm(val_loader)):
            ...
            masked_heatmap = zone_target.squeeze() * it_heatmaps
            RMA = masked_heatmap.sum(dim=(-2,-1)) / (it_heatmaps.sum(dim=(-2,-1)) + 1e-5)
            if zone_target[0, 0].sum() != 0:
                RMA_batches['pz'].append(RMA[0])
            if zone_target[0, 1].sum() != 0:
                RMA_batches['cz'].append(RMA[1])
    mean_pz_RMA = torch.stack(RMA_batches['pz']).mean(dim=0)
    mean_cz_RMA = torch.stack(RMA_batches['cz']).mean(dim=0)
    os.system(
        f'cd {log_dir} && touch localization_result.txt && echo "pz RMAs: {mean_pz_RMA}, tz RMAs: {mean_cz_RMA}, average: {(mean_pz_RMA + mean_cz_RMA)/2}" >> localization_result.txt')
...
if __name__ == '__main__':
    ...
    log_dir = f'./new_log/eval/Seg_Align/{os.path.basename(ckpt_path)}/MSD'
