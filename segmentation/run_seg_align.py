...
from config_seg_align import (AP_LIST, CURRENT_FOLD, FOLD_NUM, INIT_TRAINER,
                              SETUP_TRAINER, VERSION, PATH_LIST, WEIGHT_PATH)
from trainer_seg_align import SemanticSeg
...
def get_cross_validation_by_sample(path_list, fold_num, current_fold):
    ...
    sample_list.sort()
    # print(f'{sample_list[0]}')
    # print(f'{path_list[0]}')
    print('number of sample:', len(sample_list), '; path list length:', len(path_list))
...
def get_parameter_number(net):
    total_num = sum(p.numel() for p in net.parameters()) / 1000000.0
    trainable_num = sum(p.numel() for p in net.parameters() if p.requires_grad) / 1000000.0
    # return {'Total': total_num, 'Trainable': trainable_num}
    return {"Total size: {:.2f}M | Trainable size: {:.2f}M".format(total_num, trainable_num)}
