...
def extract_lesion_candidates_static(
    ...
    for idx in range(1, num_blobs+1):
        ...
        if np.count_nonzero(hard_mask) <= min_voxels_detection:
            ...
            continue
        intersect = np.count_nonzero((gland_output > threshold) & (hard_mask > 0))
        lesion_size = np.count_nonzero(hard_mask)
        ratio = intersect/lesion_size
        if ratio < 0.5:
            # remove lesion that is outside the gland
            blobs_index[hard_mask.astype(bool)] = 0
            print('remove one lesion')
            continue
