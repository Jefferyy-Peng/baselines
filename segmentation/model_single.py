...
class ModelEmb(nn.Module):
    def __init__(self):
        ...
        self.backbone = HarDNet(depth_wise=False, arch=85)
        d, f = self.backbone.full_features, self.backbone.features
...
if __name__ == "__main__":
    ...
    model = SegDecoderCNN(num_classes=1, num_depth=4).cuda()
    print("==> Total parameters (reference): {:.2f}M".format(sum(p.numel() for p in model.parameters()) / 1000000.0))
    # x = torch.randn((10, 3, 512, 512)).cuda()
    x = torch.randn((10, 256, 64, 64)).cuda()
    P = model(x)
    print(P.shape)
