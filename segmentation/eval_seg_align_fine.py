...
def plot_eval_multi_level(net, val_path, ckpt_path, log_dir, device, activation, is_post_process):
    ...
    with torch.no_grad():
        for step, (sample, pid, slice) in enumerate(tqdm(val_loader)):
            ...
            with autocast(False):
                if activation:
                    output, visual_embed, text_embed, logits_scale, it_heatmaps = net(data, text_tokens.squeeze(0), batched_concept_embed)
                    logits = torch.sigmoid(output)
                else:
                    logits = net(data, text_tokens.squeeze(0), batched_concept_embed)
...
if __name__ == '__main__':
    ...
    log_dir = f'./new_log/eval/Seg_Align/{os.path.basename(ckpt_path)}/picai'
    # log_dir = './new_log/eval/UNet3LevelALLDataEqualRate'

    start_time = time.time()
    if PHASE == 'seg':
        plot_eval_multi_level(net, val_path, ckpt_path, log_dir, 'cuda:1', activation, is_post_process)
    elif PHASE == 'concept localization':
        eval_concept_localization(net, val_path, ckpt_path, log_dir, 'cuda:0', activation)
    else:
