...
class _Fact_tt_Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, FacTu, FacTv) -> torch.Tensor:
        ...
        qkv = self.Attention.qkv(x, FacTu, FacTv).reshape(B, H * W, 3, self.Attention.num_heads, -1).permute(2, 0, 3, 1,
                                                                                                             4)
...
class Fact_tt_Sam(nn.Module):
    ...
    def save_parameters(self, filename: str) -> None:
        ...
        merged_dict = {**a_tensors, **b_tensors, **FacTu_tensors, **FacTv_tensors, **prompt_encoder_tensors,
                       **mask_decoder_tensors, **adapter_tensor}
