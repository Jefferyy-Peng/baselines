...
def plot_eval_multi_level(net, val_path, lesion_ckpt_path, zone_ckpt_path, gland_ckpt_path, log_dir, device):
    ...
    with torch.no_grad():
        for step, (sample, pid, slice) in enumerate(tqdm(val_loader)):
            ...
            with autocast(False):
                lesion_output = (lesion_net(data) > 0.5)
                zone_output = (zone_net(data) > 0.5)
                gland_output = (gland_net(data) > 0.5)
                # if isinstance(output, tuple):
                #     output = output[0]
...
if __name__ == '__main__':
    ...
    lesion_ckpt_path = './new_ckpt/{}/{}/fold5'.format('seg','UNet_empty_images')
    zone_ckpt_path = './new_ckpt/{}/{}/fold1'.format('seg','UNet_no_empty_images_zone')
    gland_ckpt_path = './new_ckpt/{}/{}/fold1'.format('seg','UNet_no_empty_images_gland')

    log_dir = './new_log/eval/UNet'
