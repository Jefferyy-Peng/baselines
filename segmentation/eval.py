...
from segmentation.TransUNet import CONFIGS, VisionTransformer
...
def plot_segmentation2D_multilevel(img2D, lesion_prev_masks, zone_prev_masks, gland_prev_masks, lesion_gt2D, zone_gt2D, gland_gt2D, save_path, count, image_dice=None):
    ...
    image_pred = add_contour(img2D[..., 0].unsqueeze(-1).expand(-1, -1, 3), lesion_prev_masks[1], zone_prev_masks[0], zone_prev_masks[1], gland_prev_masks[1])
    image_gt = add_contour(img2D[..., 0].unsqueeze(-1).expand(-1, -1, 3), lesion_gt2D[1].squeeze(0), zone_gt2D.squeeze(0)[0], zone_gt2D.squeeze(0)[1], gland_gt2D[1].squeeze(0))
    lesion_dice = compute_dice(lesion_prev_masks[1].int(), lesion_gt2D[1].squeeze(0))
    pz_dice = compute_dice(zone_prev_masks[0].int(), zone_gt2D.squeeze(0)[0])
    tz_dice = compute_dice(zone_prev_masks[1].int(), zone_gt2D.squeeze(0)[1])
    gland_dice = compute_dice(gland_prev_masks[1].int(), gland_gt2D[1].squeeze(0))
...
def plot_eval_multi_level(net, val_path, lesion_ckpt_path, zone_ckpt_path, gland_ckpt_path, log_dir, device):
    lesion_ckpt_file = os.path.join(lesion_ckpt_path, search_ckpt_path(lesion_ckpt_path))
    zone_ckpt_file = os.path.join(zone_ckpt_path, search_ckpt_path(zone_ckpt_path))
    gland_ckpt_file = os.path.join(gland_ckpt_path, search_ckpt_path(gland_ckpt_path))
    lesion_pid = pickle.load(open('./lesion_pid.p', 'rb'))
    zone_pid = pickle.load(open('./zone_pid.p', 'rb'))
    gland_pid = pickle.load(open('./gland_pid.p', 'rb'))

    lesion_state_dict = torch.load(lesion_ckpt_file, map_location=device)['state_dict']
    zone_state_dict = torch.load(zone_ckpt_file, map_location=device)['state_dict']
    gland_state_dict = torch.load(gland_ckpt_file, map_location=device)['state_dict']
    lesion_net = copy.deepcopy(net)
    lesion_net.load_state_dict(lesion_state_dict)
    zone_net = copy.deepcopy(net)
    zone_net.load_state_dict(zone_state_dict)
    gland_net = copy.deepcopy(net)
    gland_net.load_state_dict(gland_state_dict)

    lesion_net.eval()
    lesion_net.to(device)
    zone_net.eval()
    zone_net.to(device)
    gland_net.eval()
    gland_net.to(device)
    ...
    with torch.no_grad():
        for step, (sample, pid, slice) in enumerate(tqdm(val_loader)):
            data = sample['image']
            lesion_target = sample['lesion_label']
            zone_target = sample['zone_label']
            gland_target = sample['gland_label']
            ...
            with autocast(False):
                lesion_output = (torch.sigmoid(lesion_net(data)) > 0.5)
                zone_output = (torch.sigmoid(zone_net(data)) > 0.5)
                gland_output = (torch.sigmoid(gland_net(data)) > 0.5)
                # if isinstance(output, tuple):
                #     output = output[0]
            plot_segmentation2D_multilevel(data.squeeze(0).permute(1, 2, 0), lesion_output.squeeze(0), zone_output.squeeze(0), gland_output.squeeze(0), lesion_target.squeeze(0).squeeze(2), zone_target.squeeze(0).squeeze(1), gland_target.squeeze(0).squeeze(2), log_dir, pid[0]+'-'+slice[0])
            count += 1


if __name__ == '__main__':
    PATH_DIR = './dataset/segdata_test/data_2d'
    PATH_LIST = glob.glob(os.path.join(PATH_DIR, '*.hdf5'))
    train_path, val_path = get_cross_validation_by_sample(PATH_LIST, 5, 1)
    config_vit = CONFIGS['R50-ViT-B_16']
    config_vit.n_classes = 2
    config_vit.n_skip = 3
    net = VisionTransformer(config_vit, img_size=384, num_classes=config_vit.n_classes)
    lesion_ckpt_path = './new_ckpt/{}/{}/fold5'.format('seg','TransUNet_empty_images')
    zone_ckpt_path = './new_ckpt/{}/{}/fold1'.format('seg','TransUNet_empty_images_zone')
    gland_ckpt_path = './new_ckpt/{}/{}/fold1'.format('seg','TransUNet_empty_images_gland')

    log_dir = './new_log/eval/TransUNet'

    plot_eval_multi_level(net, val_path, lesion_ckpt_path, zone_ckpt_path, gland_ckpt_path, log_dir, 'cuda:1')