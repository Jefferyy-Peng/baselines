...
from segmentation.MedSAMAuto import MedSAMAUTO,MedSAMAUTOZONE
...
def add_contour(original_image, mask_lesion, mask_pz, mask_cz, mask_gland, random_color=False, contour_thickness=2,):
    ...
    contours_lesion, _ = cv2.findContours(mask_uint8_lesion.squeeze(0), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    ...
    contours_gland, _ = cv2.findContours(mask_uint8_gland.squeeze(0), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
...
def plot_eval_multi_level(net, zone_net, val_path, lesion_ckpt_path, zone_ckpt_path, gland_ckpt_path, log_dir, device):
    ...
    zone_net = copy.deepcopy(zone_net)
...
if __name__ == '__main__':
    ...
    lesion_ckpt_path = './new_ckpt/{}/{}/fold1'.format('seg','MedSAMAuto_no_empty_freeze_image_encoder_adam_lr_0.0001_weight_decay_0.001')
