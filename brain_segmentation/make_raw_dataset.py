...
def convert_syMRI_to_2d(input_dir, output_dir, with_label=False):
    ...
    for sample_idx, name in enumerate(tqdm(sample_names, desc="Processing samples")):
        ...
        t1 = nib.load(os.path.join(sample_path, "T1.nii.gz")).get_fdata()
        t2 = nib.load(os.path.join(sample_path, "T2.nii.gz")).get_fdata()
        pd = nib.load(os.path.join(sample_path, "PD.nii.gz")).get_fdata()
        ...
        for i in range(t1.shape[2]):
            slice_dict = {
                't1': torch.tensor(t1[:, :, i], dtype=torch.float32).unsqueeze(0),
                't2': torch.tensor(t2[:, :, i], dtype=torch.float32).unsqueeze(0),
                'pd': torch.tensor(pd[:, :, i], dtype=torch.float32).unsqueeze(0)
            }
            if with_label:
...
# Example usage:
convert_syMRI_to_2d('/home/yxpengcs/Datasets/MRI/CHDI_Multi_Contrast/SyMRI_processed_DL/', "dataset/SyMRI_raw", with_label=True)
